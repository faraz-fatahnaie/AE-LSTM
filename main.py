...
def train_ae(params, dataset_name):
    ...
    if os.path.isfile(ae_path):
        ...
        ae = Sequential()
        ae.add(LSTM(hidden_size, input_shape=(1, X_train.shape[2]), return_sequences=True))
        ae.add(LSTM(X_train.shape[2], return_sequences=True))
...
def train_cf(params):
    ...
    y = LSTM(units=params['unit1'], activation='tanh', return_sequences=True)(model_input)
    y = Dropout(params['dropout'])(y)
    y = LSTM(units=params['unit2'], activation='tanh')(y)
